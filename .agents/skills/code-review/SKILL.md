...
When running CodeRabbit through a shell tool, use a long timeout, preferably 900000 ms. If CodeRabbit times out while still `reviewing`, treat the review as incomplete, not clean. If it returns `rate_limit`, stop and report the wait time instead of retrying immediately.
